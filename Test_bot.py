...
async def set_growth(message, state):
    ...
    await message.answer(f'Целых {message.text} лет...и как ты справляешься с такими высокими технологиями в твоем-то возрасте? {growth_}')
...
async def get_buying_list(message):
    with open('img/creatine.jpg', 'rb') as file:
        await message.answer_photo(file, caption=f'Название: {product1.name} | Описание: {product1.info} | Цена: {product1.price}\n')
    with open('img/prot.jpg', 'rb') as file:
        await message.answer_photo(file, caption=f'Название: {product2.name} | Описание: {product2.info} | Цена: {product2.price}\n')
    with open('img/l-car.jpg', 'rb') as file:
        await message.answer_photo(file, caption=f'Название: {product3.name} | Описание: {product3.info} | Цена: {product3.price}\n')
    with open('img/bcaa.jpg', 'rb') as file:
        await message.answer_photo(file, caption=f'Название: {product4.name} | Описание: {product4.info} | Цена: {product4.price}\n')
